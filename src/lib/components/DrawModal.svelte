  ...
  <div class="bg-white rounded-xl shadow-2xl w-full max-w-md overflow-hidden flex flex-col fade-up">
    ...
    <div class="px-4 py-3 border-b border-zinc-100 flex items-center justify-between bg-zinc-50">
      <div class="flex items-center gap-2">
        <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" class="text-indigo-600"><path d="M12 20h9"/><path d="M16.5 3.5a2.121 2.121 0 0 1 3 3L7 19l-4 1 1-4L16.5 3.5z"/></svg>
        <h3 class="font-semibold text-zinc-800">{$t("canvas")} - {className}</h3>
      </div>
      <button on:click={close} class="text-zinc-400 hover:text-zinc-600 p-1 rounded-md hover:bg-zinc-200 transition-colors" aria-label="Close draw modal">
        <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" y1="6" x2="18" y2="18"></line></svg>
      </button>
    </div>

    <!-- Canvas Area -->
    <div class="relative bg-zinc-900 aspect-square flex items-center justify-center overflow-hidden">
      <canvas
    ...
    <!-- Controls -->
    <div class="p-4 flex items-center justify-center gap-3 bg-white">
      <button
        on:click={saveCurrentDrawing}
        disabled={!hasStrokes}
        class="w-full max-w-[220px] py-2 px-4 bg-indigo-600 hover:bg-indigo-700 disabled:bg-indigo-300 text-white text-sm font-medium rounded shadow-sm hover:shadow transition-all active:scale-95 flex items-center justify-center gap-2"
      >
    ...
    <div class="px-5 py-3 bg-zinc-50 border-t border-zinc-100">
      <p class="text-xs font-semibold text-zinc-500 uppercase tracking-wide mb-2">{$t("draw_saved_drawings")} ({savedImages.length})</p>
      <div class="flex gap-1.5 overflow-x-auto no-scrollbar">
        {#each savedImages as img, idx}
          <div class="relative group/thumb w-10 h-10 rounded overflow-hidden border border-zinc-200 shrink-0 bg-black">
            <img src={img} class="w-full h-full object-cover" alt="drawing {idx + 1}" />
            <button on:click={() => removeImage(idx)} class="absolute inset-0 bg-red-500/80 text-white opacity-0 group-hover/thumb:opacity-100 flex items-center justify-center transition-opacity" aria-label="Remove drawing">
    ...
    <!-- Footer -->
    <div class="px-5 py-4 border-t border-zinc-100 flex justify-end gap-2 bg-zinc-50">
      ...
      <button on:click={saveAndClose} disabled={savedImages.length === 0 && !hasStrokes} class="px-4 py-2 text-sm font-medium bg-indigo-600 hover:bg-indigo-700 disabled:bg-indigo-300 text-white rounded-lg transition-colors shadow-sm">
        {$t("draw_add_samples")}
      </button>
    </div>
  </div>
